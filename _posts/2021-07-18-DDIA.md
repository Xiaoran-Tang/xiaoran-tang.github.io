...
- 基于**数据库**的数据传递
  - 使用数据库**查询语言**直接读写，各读写进程彼此独立
  - 不同的进程会有数据库schema版本差异
    - 小心，旧代码可能不知道新版本的存在，在存储时丢掉新字段
...
将一个大的数据库拆分成多个较小的子集/分区，不同的分区分配给不同的节点

复制和分区经常组合使用：对于某个表，节点1保存分区1的副本1，节点2保存分区2的副本1，节点3保存分区1的副本2，节点4保存分区2的副本2
